...
public class DigRight extends Dig {

  public DigRight(SpriteInterface[][] sprites, int SET_SIZE,
      PanelInterface panel) {
    super(sprites, SET_SIZE, panel);
  }

  public SpriteInterface[][] goRight(int col, int row, SpriteInterface sprite,
      SpriteInterface[][] sprites, PanelInterface panel, AudioInterface audio) {
    try {
      this.image = ImageIO.read(new File("image/d1.png"));
    } catch (IOException e) {
      e.printStackTrace();
    }
